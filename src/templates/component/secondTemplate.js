...
const SecondTemplate = forwardRef(({ data, fontSizes }, ref) => {
  ...
  const dividerColor = data?.dividerColor || `${secondaryColor}44`;
  const surfaceColor = data?.surfaceColor || '#ffffff';
  const textColor = data?.textColor || '#0f172a';
  const titleColor = data?.titleColor || '#ffffff';
  const subtitleColor = data?.subtitleColor || '#e2e8f0';
  const sectionTitleColor = data?.sectionTitleColor || '#475569';
  ...
  return (
    <div
      ref={ref}
      className="w-[794px] min-h-[1123px] text-slate-900 mx-auto border break-words page-break-inside-avoid"
      style={{ color: textColor, backgroundColor: surfaceColor, borderColor: dividerColor }}
    >
      <div
        className="px-10 pt-10 pb-6"
        style={{ backgroundImage: `linear-gradient(90deg, ${primaryColor}, ${secondaryColor})`, color: headerTextColor }}
      >
        ...
        <div className="mt-6 grid grid-cols-2 gap-x-6 gap-y-2">
          {shouldShow(data.showEmail) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <Mail size={iconSize('email', 14)} className="mt-0.5 shrink-0" /> : null}
              <a href={hasText(data.email) ? `mailto:${data.email}` : undefined} className="break-all hover:underline" style={fontStyle('email', 14)}>{safeText(data.email, t('template.emailPlaceholder'))}</a>
            </div>
          ) : null}
          {shouldShow(data.showPhone) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <Phone size={iconSize('phone', 14)} className="mt-0.5 shrink-0" /> : null}
              <span className="break-words" style={fontStyle('phone', 14)}>{safeText(data.phone, t('template.phonePlaceholder'))}</span>
            </div>
          ) : null}
          {shouldShow(data.showWebsite) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <Globe size={iconSize('website', 14)} className="mt-0.5 shrink-0" /> : null}
              <a href={toExternalUrl(data.website)} target="_blank" rel="noopener noreferrer" className="break-all hover:underline" style={fontStyle('website', 14)}>{safeText(data.website, t('template.websitePlaceholder'))}</a>
            </div>
          ) : null}
          {shouldShow(data.showLocation) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <MapPin size={iconSize('location', 14)} className="mt-0.5 shrink-0" /> : null}
              <span className="break-words" style={fontStyle('location', 14)}>{safeText(data.location, t('template.locationPlaceholder'))}</span>
            </div>
          ) : null}
          {shouldShow(data.showLinkedin) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <Linkedin size={iconSize('linkedin', 14)} className="mt-0.5 shrink-0" /> : null}
              <a href={toExternalUrl(data.linkedin)} target="_blank" rel="noopener noreferrer" className="break-all hover:underline" style={fontStyle('linkedin', 14)}>{safeText(data.linkedin, t('template.linkedinPlaceholder'))}</a>
            </div>
          ) : null}
          {shouldShow(data.showGithub) ? (
            <div className="flex items-start gap-2 min-w-0">
              {showContactIcons ? <Github size={iconSize('github', 14)} className="mt-0.5 shrink-0" /> : null}
              <a href={toExternalUrl(data.github)} target="_blank" rel="noopener noreferrer" className="break-all hover:underline" style={fontStyle('github', 14)}>{safeText(data.github, t('template.githubPlaceholder'))}</a>
            </div>
          ) : null}
        </div>
      </div>

      <div className="grid grid-cols-[1.15fr_0.85fr] gap-8 px-10 py-8">
        <div>
          ...
          <section className="mb-7">
            ...
            <div className="mt-3 space-y-4">
              {data.experience?.length > 0 ? (
                data.experience.map((exp, index) => (
                  <div key={index}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold" style={fontStyle('experienceRole', 14)}>
                          {exp.role || t('template.role')}
                        </p>
                        <p style={{ ...fontStyle('experienceCompany', 14), color: mutedTextColor }}>
                          {exp.company || t('template.company')}
                        </p>
                      </div>
                ...
                <p className="text-sm text-slate-500">{t('template.experiencePlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
            ...
            <div className="mt-3 space-y-4">
              {data.projects?.length > 0 ? (
                data.projects.map((project, index) => (
                  <div key={index}>
                    <p className="font-semibold" style={fontStyle('projectName', 14)}>
                      {project.name || t('template.projectName')}
                    </p>
                    ...
                    {project.link ? (
                      ...
                      <p className="break-all" style={{ ...fontStyle('projectLink', 12), color: linkColor }}>
                        {t('template.websitePlaceholder')}
                      </p>
                    )}
                    <p className="mt-1 whitespace-pre-line" style={fontStyle('projectDescription', 14)}>
                      {project.description || t('template.projectsPlaceholder')}
                    </p>
                  </div>
                ))
              ) : (
                <p className="text-sm text-slate-500">{t('template.projectsPlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
          ...
          <section className="mb-7">
            ...
            <div className="mt-3 space-y-2">
              {data.skills?.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">{t('template.skillsPlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
            ...
            <div className="mt-3 space-y-2">
              {data.education?.length > 0 ? (
                data.education.map((edu, index) => (
                  <div key={index}>
                    <p className="font-semibold" style={fontStyle('educationSchool', 14)}>
                      {edu.school || t('template.educationSchoolPlaceholder')}
                    </p>
                    <p style={{ ...fontStyle('educationDegree', 12), color: mutedTextColor }}>
                      {edu.degree || t('template.educationDegreePlaceholder')}
                    </p>
                    <p style={{ ...fontStyle('educationYear', 12), color: mutedTextColor }}>
                      {edu.year || t('template.educationYearPlaceholder')}
                    </p>
                    <p style={{ ...fontStyle('educationYear', 12), color: mutedTextColor }}>
                ...
                <p className="text-sm text-slate-500">{t('template.educationPlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
          ...
          <section className="mb-7">
            ...
            <div className="mt-3 space-y-1">
              {data.languages?.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">{t('template.languagesPlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
            ...
            <div className="mt-3 space-y-2">
              {data.certifications?.length > 0 ? (
                data.certifications.map((cert, index) => (
                  <div key={index}>
                    <p className="font-semibold" style={fontStyle('certificationName', 14)}>
                      {cert.name || t('template.certificationNamePlaceholder')}
                    </p>
                    <p style={{ color: mutedTextColor }}>
                      <span style={fontStyle('certificationIssuer', 12)}>
                        {cert.issuer || t('template.certificationIssuerPlaceholder')}
                      </span>
                      {' | '}
                      <span style={fontStyle('certificationYear', 12)}>
                ...
                <p className="text-sm text-slate-500">{t('template.certificationsPlaceholder')}</p>
              )}
            </div>
          </section>
          ) : null}
